...
def main():
    ...
    output_path = input_path.split('.wav')[0] + '_enhanced_ss' + process_type + '.wav'
    wav, sample_rate = audio.load_wav(input_path, sr=None)
    wav = wav / np.abs(wav).max() * 0.999

    clean_wav = speech_enhancement.spectral_subtraction(wav, sample_rate=sample_rate, process_type=process_type)
